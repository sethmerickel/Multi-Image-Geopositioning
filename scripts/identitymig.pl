...
#$truthgp = mkmat(3,1, $truthx, $truthy, $truthz);
...
for $n (@ns) {
  for (1..$opt{n}) {
    ...
    $ENV{IDENTITY_WEIGHT} = 1;
    
    $gp0 = mkmat(3,1, 5,5,5);
    ($gp, $cov, $refvar) = wvmig($hsh, $gp0, @sample);
    $toprint .= join ',', 'M1G', $n, flatten($gp), $refvar,
                           (flatten($cov))[0,1,2,4,5,8], '';
...
$gp0 = mkmat(3,1, 5,5,5);
($gp1, $cov1, $refvar1) = wvmig($hsh, $gp0, @iids);
$cov2 = $cov1 * $refvar1;
$toprint .= join ',', 'MIG', $n, flatten($gp), $refvar,
    (flatten($cov))[0,1,2,4,5,8], '';
